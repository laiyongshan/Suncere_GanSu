...
//        	  Log.e("hjo", "调用if 的 颜色值");
        ...
        if (weekDayTextAppearance != null) {
            pagerView.setWeekDayTextAppearance(weekDayTextAppearance);                                               //这里修改了
        }
...
//		}
//        mlist.add(new CalendarDay().from(2016, 10, 10));
//        pagerView.setSelectedDates(mlist);

//        Log.e("hjolalala", "这里是什么时候调用的：instantiateItem");
        pagerView.setSelectDateColors(mlistColors);
    ...
    //我的拓展
    public void setSelectDayesBgColors(List<Integer>listColors){
//    	Log.e("hjo setSelectDayesBgColors", "添加选择的颜色 setSelectDayesBgColors"+listColors.size());
        mlistColors.clear();
...
//    	Log.e("hjo ", "CalendarPagerAdapter.setWeekDayTextAppearance  ");
        if (taId == 0) {
            return;
        }
    ...
    public void ovellClearSelections(){
        selectedDates.clear();
...
//    	Log.e("hjikkkkkkkkkk", "清除数据后"+selectedDates.toString());
        if (selected) {
